...
def _black_vol_handle(volatility: float = 0.20) -> ql.BlackVolTermStructureHandle:
    return qlBlackConstantVol(
        _reference_date(), ql.TARGET(), volatility, _day_counter()
    )
...
def test_fixed_local_vol_surface_extrapolation_converter():
    assert (
        qFixedLocalVolSurfaceExtrapolation.__wrapped__("constant")
        == ql.FixedLocalVolSurface.ConstantExtrapolation
    )
    assert (
        qFixedLocalVolSurfaceExtrapolation.__wrapped__("DEFAULT")
        == ql.FixedLocalVolSurface.InterpolatorDefaultExtrapolation
    )

    with pytest.raises(ValueError, match="Cannot convert"):
        qFixedLocalVolSurfaceExtrapolation.__wrapped__("invalid")
...
def test_qlLocalConstantVol_returns_handle_with_constant_local_vol():
    ...
    assert lvol_tsh.localVol(qlDate(2024, 7, 2), 100.0, True) == pytest.approx(
        local_vol
    )
...
def test_qlLocalVolSurface_and_qlNoExceptLocalVolSurface_create_valid_handles():
    ...
    lvol_surface = qlLocalVolSurface(
        black_vol_tsh, risk_free_ytsh, dividend_ytsh, underlying
    )
    noexcept_surface = qlNoExceptLocalVolSurface(
        black_vol_tsh, risk_free_ytsh, dividend_ytsh, underlying, 0.30
    )
...
def test_qlFixedLocalVolSurface_returns_handle_and_matches_grid_knots():
    ...
    assert isinstance(lvol_tsh, ql.LocalVolTermStructureHandle)
    assert lvol_tsh.currentLink() is not None
    assert lvol_tsh.localVol(expiry1, strikes[0], True) == pytest.approx(
        local_vol_matrix[0][0]
    )
    assert lvol_tsh.localVol(expiry2, strikes[1], True) == pytest.approx(
        local_vol_matrix[1][1]
    )
