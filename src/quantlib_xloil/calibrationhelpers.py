...
def qlBlackCalibrationHelperImpliedVolatility(
    ...
    return helper.impliedVolatility(
        target_value, accuracy, max_evaluations, min_volatility, max_volatility
    )
...
def qlSwaptionHelper(
    ...
    if isinstance(exercise, str):
        exercise = qPeriod.__wrapped__(exercise)
    else:
        exercise = qDate.__wrapped__(exercise)
    #
    if isinstance(swap_length, str):
        # (Period, Period) and (Date, Period)
        swap_length = qPeriod.__wrapped__(swap_length)
    if isinstance(exercise, ql.Date) and not isinstance(swap_length, ql.Period):
        # (Date, Date)
        swap_length = qDate.__wrapped__(swap_length)
    #
